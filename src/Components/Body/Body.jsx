...
const Body = () => {
    ...
    const filterLocalData =(id)=>{
            const findout = localdata.filter(data => data.id != id);
            const newarray = JSON.stringify(findout);
    ...
    return (
        <div className='grid grid-cols-5 w-11/12 mx-auto gap-5 my-5'>
            ...
            <div className='border h-fit p-5 rounded-md'>
                ...
                <div className='my-5'>
                    <h1 className='font-semibold border px-2 py-1 rounded-md mb-3 w-fit'>Total items: <span className='text-sm'>{localdata ? localdata.length:0}</span></h1>

                    <div className='text-xs'>{localdata && localdata.map(data=> <Cart cartData={data} filterdata ={filterLocalData}></Cart>)}</div>
                </div>

                <h1 className='border-t pt-2 font-bold'>Total Price:<span className='mx-5 font-thin'>${totalMoney? totalMoney: 0}</span></h1>
            </div>
